TrainerSpellsBuiltin_ProfessionRecipe = TrainerSpellsBuiltin_ProfessionRecipe or {}
TrainerSpellsBuiltin_ProfessionRecipe.Engineering = TrainerSpellsBuiltin_ProfessionRecipe.Engineering or {
    [75] = {
        [3928] = {
            icon = "Interface\\Icons\\inv_crate_01",
            source = "World drop",
        },
    },
    [100] = {
        [3933] = {
            icon = "Interface\\Icons\\inv_misc_urn_01",
            source = "World drop",
        },
        [8339] = {
            icon = "Interface\\Icons\\inv_misc_bomb_06",
            source = "World drop",
        },
    },
    [120] = {
        [3939] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_07",
            source = "Sold by Fradd Swiftgear (Wetlands), Jinky Twizzlefixxit (Thousand Needles)",
        },
        [3940] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "World drop",
        },
    },
    [125] = {
        [3944] = {
            icon = "Interface\\Icons\\inv_gizmo_01",
            source = "Dropped by Mekgineer Thermaplugg and STX-04/BD (Gnomeregan)",
        },
        [9269] = {
            icon = "Interface\\Icons\\inv_misc_pocketwatch_01",
            source = "Sold by Gearcutter Cogspinner (Ironforge), Jinky Twizzlefixxit (Thousand Needles); rare drop from Mekgineer Thermaplugg/STX-04/BD (Gnomeregan)",
        },
        [26416] = {
            icon = "Interface\\Icons\\inv_misc_missilesmall_blue",
            source = "Found in Small Rocket Recipes (Lunar Festival)",
        },
        [26417] = {
            icon = "Interface\\Icons\\inv_misc_missilesmall_green",
            source = "Found in Small Rocket Recipes (Lunar Festival)",
        },
        [26418] = {
            icon = "Interface\\Icons\\inv_misc_missilesmall_red",
            source = "Found in Small Rocket Recipes (Lunar Festival)",
        },
    },
    [140] = {
        [3952] = {
            icon = "Interface\\Icons\\inv_gizmo_07",
            source = "Sold by Fradd Swiftgear (Wetlands), Namdo Bizzfizzle, Gagsprocket (Barrens)",
        },
    },
    [145] = {
        [3954] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_06",
            source = "World drop",
        },
    },
    [150] = {
        [23066] = {
            icon = "Interface\\Icons\\spell_fire_fireball02",
            source = "Sold by Sovik (Orgrimmar)",
            faction = "Horde",
        },
        [23067] = {
            icon = "Interface\\Icons\\spell_ice_magicdamage",
            source = "Sold by Darian Singh (Stormwind), Gearcutter Cogspinner (Ironforge)",
            faction = "Alliance",
        },
        [23068] = {
            icon = "Interface\\Icons\\spell_nature_abolishmagic",
            source = "Sold by Gagsprocket (Barrens), Crazk Sparks (Booty Bay)",
        },
    },
    [155] = {
        [3957] = {
            icon = "Interface\\Icons\\inv_gizmo_01",
            source = "Sold by Rizz Loosebolt (Alterac Mountains), Super-Seller 680 (Desolace)",
        },
    },
    [160] = {
        [3959] = {
            icon = "Interface\\Icons\\inv_misc_spyglass_02",
            source = "Dropped by Mekgineer Thermaplugg and STX-04/BD (Gnomeregan)",
        },
    },
    [165] = {
        [3960] = {
            icon = "Interface\\Icons\\inv_musket_01",
            source = "World drop",
        },
        [9273] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_10",
            source = "Sold by Veenix (Stonetalon Mountains), Kzixx (Duskwood), Zixil (Hillsbrad), Super-Seller 680 (Desolace)",
        },
    },
    [175] = {
        [12587] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "World drop",
        },
        [26420] = {
            icon = "Interface\\Icons\\inv_misc_missilelarge_blue",
            source = "Found in Large Rocket Recipes (Lunar Festival)",
        },
        [26421] = {
            icon = "Interface\\Icons\\inv_misc_missilelarge_green",
            source = "Found in Large Rocket Recipes (Lunar Festival)",
        },
        [26422] = {
            icon = "Interface\\Icons\\inv_misc_missilelarge_red",
            source = "Found in Large Rocket Recipes (Lunar Festival)",
        },
    },
    [180] = {
        [3979] = {
            icon = "Interface\\Icons\\inv_misc_spyglass_02",
            source = "Sold by Mazk Snipeshot (Booty Bay), Super-Seller 680 (Desolace)",
        },
    },
    [185] = {
        [3966] = {
            icon = "Interface\\Icons\\inv_helmet_44",
            source = "World drop, also found in chests and lockboxes",
        },
        [8243] = {
            icon = "Interface\\Icons\\inv_misc_ammo_bullet_01",
            source = "Dropped by Mekgineer Thermaplugg and STX-04/BD (Gnomeregan); quest reward: \"Flash Bomb Recipe\"",
        },
    },
    [190] = {
        [21940] = {
            icon = "Interface\\Icons\\spell_frost_windwalkon",
            source = "Found in Smokywood Pastures Special Gift and Ticking Present (Feast of Winter Veil)",
        },
    },
    [195] = {
        [3968] = {
            icon = "Interface\\Icons\\inv_shield_08",
            source = "World drop",
        },
    },
    [200] = {
        [3969] = {
            icon = "Interface\\Icons\\inv_misc_head_dragon_01",
            source = "Sold by Gnaz Blunderflame (Stranglethorn Vale)",
        },
        [3971] = {
            icon = "Interface\\Icons\\inv_gizmo_01",
            source = "Sold by Zan Shivsproket (Alterac Mountains); rare drop from Mekgineer Thermaplugg (Gnomeregan)",
        },
        [3972] = {
            icon = "Interface\\Icons\\inv_misc_urn_01",
            source = "World drop",
        },
        [23069] = {
            icon = "Interface\\Icons\\inv_misc_bomb_03",
            source = "Sold by Blizrik Buckshot (Gadgetzan, Tanaris)",
        },
    },
    [205] = {
        [15628] = {
            icon = "Interface\\Icons\\inv_misc_bomb_04",
            source = "Found in Goblin Engineer's Renewal Gift",
            requires = {20222}
        },
        [15633] = {
            icon = "Interface\\Icons\\inv_misc_idol_02",
            source = "Found in Gnome Engineer's Renewal Gift",
            requires = {20219}
        },
    },
    [210] = {
        [12597] = {
            icon = "Interface\\Icons\\inv_misc_spyglass_02",
            source = "Sold by Knaz Blunderflame (Stranglethorn Vale), Yuka Screwspigot (Burning Steppes)",
        },
    },
    [220] = {
        [12607] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "World drop, also found in chests and lockboxes",
        },
        [12614] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_07",
            source = "World drop",
        },
    },
    [225] = {
        [12615] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "World drop, also found in chests and lockboxes",
        },
        [12616] = {
            icon = "Interface\\Icons\\inv_misc_cape_11",
            source = "World drop",
        },
        [26423] = {
            icon = "Interface\\Icons\\inv_misc_missilesmallcluster_blue",
            source = "Found in Cluster Rocket Recipes (Lunar Festival)",
        },
        [26424] = {
            icon = "Interface\\Icons\\inv_misc_missilesmallcluster_green",
            source = "Found in Cluster Rocket Recipes (Lunar Festival)",
        },
        [26425] = {
            icon = "Interface\\Icons\\inv_misc_missilesmallcluster_red",
            source = "Found in Cluster Rocket Recipes (Lunar Festival)",
        },
        [26442] = {
            icon = "Interface\\Icons\\inv_musket_04",
            source = "Quest reward: \"Firework Launcher\" (Lunar Festival)",
        },
    },
    [230] = {
        [12617] = {
            icon = "Interface\\Icons\\inv_helmet_49",
            source = "Sold by Jubie Gadgetspring (Azshara)",
        },
    },
    [240] = {
        [12620] = {
            icon = "Interface\\Icons\\inv_misc_spyglass_02",
            source = "World drop, also found in chests and lockboxes",
        },
    },
    [250] = {
        [12624] = {
            icon = "Interface\\Icons\\inv_misc_head_dragon_01",
            source = "Sold by Ruppo Zipcoil (Hinterlands)",
        },
        [23507] = {
            icon = "Interface\\Icons\\spell_holy_holybolt",
            source = "Sold by Zorbin Fandazzle (Feralas), unlocked after quests Zapped Giants & Fuel for the Zapping",
        },
    },
    [260] = {
        [19790] = {
            icon = "Interface\\Icons\\inv_misc_bomb_08",
            source = "Sold by Gearcutter Cogspinner (Ironforge), Sovik (Orgrimmar)",
        },
        [19791] = {
            icon = "Interface\\Icons\\inv_gizmo_04",
            source = "Sold by Gearcutter Cogspinner (Ironforge), Sovik (Orgrimmar)",
        },
        [19792] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_07",
            source = "World drop",
        },
        [23071] = {
            icon = "Interface\\Icons\\inv_staff_18",
            source = "Sold by Mazk Snipeshot (Stranglethorn Vale, Booty Bay)",
        },
        [23077] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_02",
            source = "Sold by Xizzer Fizzbolt (Winterspring/Everlook)",
        },
        [23129] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_08",
            source = "Dropped by Weapon Technician (Blackrock Depths)",
            requires = {20219}
        },
    },
    [265] = {
        [19793] = {
            icon = "Interface\\Icons\\inv_misc_monsterhead_03",
            source = "World drop, also found in lockboxes",
        },
        [23078] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_11",
            source = "Dropped by Plugger Spazzring (Blackrock Depths)",
        },
        [23096] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_13",
            source = "Dropped by Weapon Technician (Blackrock Depths)",
        },
    },
    [270] = {
        [19794] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "Dropped by Spitelash Siren (Azshara)",
        },
    },
    [275] = {
        [19795] = {
            icon = "Interface\\Icons\\inv_gizmo_pipe_04",
            source = "Sold by Xizzer Fizzbolt (Winterspring/Everlook)",
        },
        [19796] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_08",
            source = "Dropped by Doomforge Craftsman (Blackrock Depths)",
        },
        [19814] = {
            icon = "Interface\\Icons\\inv_crate_02",
            source = "Sold by Xizzer Fizzbolt (Winterspring/Everlook)",
        },
        [23079] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_09",
            source = "Found in Gordok Tribute (Dire Maul)",
        },
        [23080] = {
            icon = "Interface\\Icons\\spell_shadow_detectinvisibility",
            source = "Sold by Xizzer Fizzbolt (Winterspring/Everlook)",
        },
        [26426] = {
            icon = "Interface\\Icons\\inv_misc_missilelargecluster_blue",
            source = "Found in Large Cluster Rocket Recipes (Lunar Festival)",
        },
        [26427] = {
            icon = "Interface\\Icons\\inv_misc_missilelargecluster_green",
            source = "Found in Large Cluster Rocket Recipes (Lunar Festival)",
        },
        [26428] = {
            icon = "Interface\\Icons\\inv_misc_missilelargecluster_red",
            source = "Found in Large Cluster Rocket Recipes (Lunar Festival)",
        },
        [26443] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_03",
            source = "Quest reward: \"Cluster Launcher\" (Lunar Festival)",
        },
        [28327] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_20",
            source = "Darkmoon Faire prize: 40 Darkmoon Faire Prize Tickets",
        },
    },
    [285] = {
        [19799] = {
            icon = "Interface\\Icons\\inv_misc_bomb_05",
            source = "Dropped by Weapon Technician (Blackrock Depths)",
        },
        [19800] = {
            icon = "Interface\\Icons\\inv_ammo_bullet_03",
            source = "World drop",
        },
        [19815] = {
            icon = "Interface\\Icons\\inv_gizmo_07",
            source = "Sold by Xizzer Fizzbolt (Winterspring/Everlook)",
        },
    },
    [290] = {
        [19819] = {
            icon = "Interface\\Icons\\inv_jewelry_amulet_07",
            source = "Dropped by Crimson Inquisitor (Stratholme)",
        },
        [19825] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "Dropped by Doomforge Arcanasmith (Blackrock Depths)",
        },
        [23081] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_04",
            source = "Dropped by Solakar Flamewreath (Blackrock Spire)",
        },
    },
    [300] = {
        [19830] = {
            icon = "Interface\\Icons\\inv_misc_head_dragon_01",
            source = "Dropped by Cobalt Mageweaver (Winterspring)",
        },
        [19831] = {
            icon = "Interface\\Icons\\spell_shadow_mindbomb",
            source = "World drop (raid bosses and level 60 mobs: Azuregos, Onyxia, Nefarian, Dire Maul, Zul'Gurub)",
        },
        [19833] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_03",
            source = "Dropped by Mossflayer Shadowhunter (Eastern Plaguelands)",
        },
        [22704] = {
            icon = "Interface\\Icons\\inv_egg_05",
            source = "Learned directly from a schematic object in the Manufactory, Blackrock Depths (not a lootable/vendor item)",
        },
        [22793] = {
            icon = "Interface\\Icons\\inv_misc_spyglass_02",
            source = "World drop",
        },
        [22795] = {
            icon = "Interface\\Icons\\inv_weapon_rifle_05",
            source = "Dropped by Molten Core bosses",
        },
        [22797] = {
            icon = "Interface\\Icons\\spell_arcane_portaldarnassus",
            source = "World drop",
        },
        [23082] = {
            icon = "Interface\\Icons\\inv_misc_enggizmos_16",
            source = "Dropped by Crimson Inquisitor (Stratholme)",
        },
        [24356] = {
            icon = "Interface\\Icons\\inv_helmet_47",
            source = "Zandalar Tribe - Honored (Rin'wosho the Trader, Zul'Gurub isle)",
        },
        [24357] = {
            icon = "Interface\\Icons\\inv_helmet_44",
            source = "Zandalar Tribe - Friendly (Rin'wosho the Trader, Zul'Gurub isle)",
        },
    },
}
