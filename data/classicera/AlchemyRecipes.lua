TrainerSpellsBuiltin_AddProfessionRecipes(
    "Alchemy",
    {
        [50] = {
            [3230] = {
                icon = "Interface\\Icons\\inv_potion_91",
                source = "World drop",
            },
            [4508] = {
                icon = "Interface\\Icons\\inv_potion_35",
                source = "Quest reward: \"Wild Hearts\" / \"Unfamiliar Foulness\"",
            },
        },
        [60] = {
            [6617] = {
                icon = "Interface\\Icons\\inv_potion_24",
                source = "Sold by vendors in Loch Modan, Westfall, Orgrimmar, The Barrens",
            },
            [2335] = {
                icon = "Interface\\Icons\\inv_potion_95",
                source = "World drop",
            },
        },
        [90] = {
            [8240] = {
                icon = "Interface\\Icons\\inv_potion_10",
                source = "World drop",
            },
        },
        [100] = {
            [7255] = {
                icon = "Interface\\Icons\\inv_potion_09",
                source = "Sold by Xandar Goodbeard (Loch Modan), Kzixx (Duskwood), Hula'mahi (The Barrens)",
            },
        },
        [110] = {
            [3172] = {
                icon = "Interface\\Icons\\inv_potion_08",
                source = "World drop",
            },
        },
        [120] = {
            [3174] = {
                icon = "Interface\\Icons\\inv_potion_12",
                source = "World drop",
            },
        },
        [135] = {
            [7256] = {
                icon = "Interface\\Icons\\inv_potion_44",
                source = "Sold by Harklan Moongrove (Ashenvale), Christoph Jeffcoat (Hillsbrad Foothills)",
            },
        },
        [140] = {
            [2333] = {
                icon = "Interface\\Icons\\inv_potion_92",
                source = "World drop",
            },
        },
        [150] = {
            [6624] = {
                icon = "Interface\\Icons\\inv_potion_04",
                source = "Sold by vendors in Darnassus, Ironforge, Orgrimmar, Desolace",
            },
            [3188] = {
                icon = "Interface\\Icons\\inv_potion_57",
                source = "World drop",
            },
        },
        [165] = {
            [3449] = {
                icon = "Interface\\Icons\\inv_potion_23",
                source = "Sold by Bliztik (Duskwood), Montarr (Thousand Needles)",
            },
            [7257] = {
                icon = "Interface\\Icons\\inv_potion_16",
                source = "Sold by Nandar Branson (Hillsbrad Foothills), Jeeda (Stonetalon Mountains)",
            },
        },
        [175] = {
            [6618] = {
                icon = "Interface\\Icons\\inv_potion_21",
                source = "Sold by Ulthir (Darnassus), Hagrus (Orgrimmar)",
            },
        },
        [180] = {
            [3451] = {
                icon = "Interface\\Icons\\inv_potion_79",
                source = "World drop",
            },
        },
        [190] = {
            [21923] = {
                icon = "Interface\\Icons\\inv_potion_03",
                source = "Rare drop from Frenzied Plaguehound (Eastern Plaguelands)",
            },
            [7258] = {
                icon = "Interface\\Icons\\inv_potion_13",
                source = "Sold by Drovnar Strongbrew (Arathi Highlands), Glyx Brewright (Stranglethorn Vale)",
            },
            [7259] = {
                icon = "Interface\\Icons\\inv_potion_06",
                source = "Sold by Glyx Brewright (Stranglethorn Vale), Alchemist Pestlezugg (Tanaris), Logannas (Feralas, Alliance) / Bronk (Feralas, Horde)",
            },
        },
        [195] = {
            [3453] = {
                icon = "Interface\\Icons\\inv_potion_01",
                source = "World drop",
            },
        },
        [200] = {
            [3454] = {
                icon = "Interface\\Icons\\inv_potion_20",
                source = "Sold by Bro'kin, Alterac Mountains",
            },
        },
        [210] = {
            [11453] = {
                icon = "Interface\\Icons\\inv_potion_16",
                source = "World drop",
            },
            [11456] = {
                icon = "Interface\\Icons\\inv_cask_02",
                source = "Learned from Goblin Rocket Fuel Recipe (crafted by Goblin Engineers)",
            },
        },
        [215] = {
            [4942] = {
                icon = "Interface\\Icons\\inv_potion_67",
                source = "Quest reward: \"Liquid Stone\"",
            },
        },
        [225] = {
            [11459] = {
                icon = "Interface\\Icons\\inv_misc_orb_01",
                source = "Sold by Alchemist Pestlezugg (Tanaris)",
            },
            [11458] = {
                icon = "Interface\\Icons\\inv_potion_34",
                source = "World drop",
            },
        },
        [235] = {
            [11464] = {
                icon = "Interface\\Icons\\inv_potion_25",
                source = "World drop",
            },
        },
        [240] = {
            [11466] = {
                icon = "Interface\\Icons\\inv_potion_28",
                source = "Drops from skeletons, southern Western Plaguelands",
            },
            [11468] = {
                icon = "Interface\\Icons\\inv_potion_14",
                source = "World drop, also found in chests and lockboxes",
            },
        },
        [245] = {
            [11472] = {
                icon = "Interface\\Icons\\inv_potion_61",
                source = "Uncommon world drop from many mobs",
            },
            [11473] = {
                icon = "Interface\\Icons\\inv_poison_mindnumbing",
                source = "Sold by Logannas / Bronk (Feralas)",
            },
        },
        [250] = {
            [11476] = {
                icon = "Interface\\Icons\\inv_potion_46",
                source = "Sold by Algernon (Undercity) / Maria Lumere (Stormwind)",
            },
            [26277] = {
                icon = "Interface\\Icons\\inv_potion_60",
                source = "Drops from Dark Iron Dwarves (Searing Gorge)",
            },
            [3175] = {
                icon = "Interface\\Icons\\inv_potion_62",
                source = "World drop",
            },
            [11477] = {
                icon = "Interface\\Icons\\inv_potion_27",
                source = "Sold by Rartar (Swamp of Sorrows), Nina Lightbrew (Blasted Lands)",
            },
        },
        [255] = {
            [17552] = {
                icon = "Interface\\Icons\\inv_potion_41",
                source = "Drops from Blackrock Slayer (Burning Steppes)",
            },
        },
        [260] = {
            [17553] = {
                icon = "Interface\\Icons\\inv_potion_74",
                source = "Sold by Ulthir (Darnassus), Algernon (Undercity)",
            },
        },
        [265] = {
            [17554] = {
                icon = "Interface\\Icons\\inv_potion_66",
                source = "Sold by Soolie Berryfizz (Ironforge) / Kor'geld (Orgrimmar)",
            },
        },
        [270] = {
            [17555] = {
                icon = "Interface\\Icons\\inv_potion_29",
                source = "Drops from Scarlet Crusade members (Eastern Plaguelands)",
            },
        },
        [275] = {
            [17187] = {
                icon = "Interface\\Icons\\inv_misc_stonetablet_05",
                source = "Sold by Alchemist Pestlezugg (Tanaris)",
            },
            [24365] = {
                icon = "Interface\\Icons\\inv_potion_45",
                source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Revered)",
            },
            [24366] = {
                icon = "Interface\\Icons\\inv_potion_83",
                source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Friendly)",
            },
            [17559] = {
                icon = "Interface\\Icons\\spell_fire_volcano",
                source = "Sold by Argent Quartermaster Hasana (Tirisfal Glades), Argent Quartermaster Lightspark (Western Plaguelands), Quartermaster Miranda Breechlock (Eastern Plaguelands) (requires The Argent Dawn - Honored)",
            },
            [17561] = {
                icon = "Interface\\Icons\\spell_nature_acid_01",
                source = "Sold by Meilosh, Felpaw Village (Felwood) (requires Timbermaw Hold - Friendly)",
            },
            [17560] = {
                icon = "Interface\\Icons\\spell_nature_strengthofearthtotem02",
                source = "Sold by Plugger Spazzring, Blackrock Depths (Grim Guzzler)",
            },
            [17562] = {
                icon = "Interface\\Icons\\spell_nature_earthbind",
                source = "Sold by Magnus Frostwake, outside Scholomance (requires Eva Sarkhoff questline)",
            },
            [17556] = {
                icon = "Interface\\Icons\\inv_potion_54",
                source = "Sold by Evie Whirlbrew (Winterspring, Everlook)",
            },
            [17557] = {
                icon = "Interface\\Icons\\inv_potion_40",
                source = "Found in Small Brown-wrapped Package",
            },
            [17563] = {
                icon = "Interface\\Icons\\spell_nature_acid_01",
                source = "World drop, also found in chests and lockboxes",
            },
            [17564] = {
                icon = "Interface\\Icons\\spell_shadow_shadetruesight",
                source = "World drop, also found in chests and lockboxes",
            },
            [17565] = {
                icon = "Interface\\Icons\\spell_nature_strengthofearthtotem02",
                source = "World drop, also found in chests and lockboxes",
            },
            [17566] = {
                icon = "Interface\\Icons\\spell_nature_abolishmagic",
                source = "World drop, also found in chests and lockboxes",
            },
        },
        [280] = {
            [17571] = {
                icon = "Interface\\Icons\\inv_potion_32",
                source = "Drops from Legashi satyrs, Jadefire Run (Felwood)",
            },
            [17570] = {
                icon = "Interface\\Icons\\inv_potion_69",
                source = "World drop, also found in chests and lockboxes",
            },
        },
        [285] = {
            [24367] = {
                icon = "Interface\\Icons\\inv_potion_07",
                source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Exalted)",
            },
            [17572] = {
                icon = "Interface\\Icons\\inv_potion_31",
                source = "World drop",
            },
            [17573] = {
                icon = "Interface\\Icons\\inv_potion_25",
                source = "World drop",
            },
        },
        [290] = {
            [24368] = {
                icon = "Interface\\Icons\\inv_potion_80",
                source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Honored)",
            },
            [17574] = {
                icon = "Interface\\Icons\\inv_potion_24",
                source = "Dropped by Firebrand Invoker and Firebrand Pyromancer (Blackrock Spire)",
            },
            [17575] = {
                icon = "Interface\\Icons\\inv_potion_20",
                source = "Dropped by Frostmaul Giant (Winterspring)",
            },
            [17576] = {
                icon = "Interface\\Icons\\inv_potion_22",
                source = "Dropped by Rotting Behemoth and Decaying Horror (Western Plaguelands)",
            },
            [17577] = {
                icon = "Interface\\Icons\\inv_potion_83",
                source = "Dropped by Cobalt Mageweaver (Winterspring)",
            },
            [17578] = {
                icon = "Interface\\Icons\\inv_potion_23",
                source = "Dropped by Dark Adept and Shadowmage (Eastern Plaguelands)",
            },
        },
        [295] = {
            [17580] = {
                icon = "Interface\\Icons\\inv_potion_76",
                source = "Sold by Magnus Frostwake (Western Plaguelands); also rare drop from Darkmaster Gandling in Scholomance",
            },
        },
        [300] = {
            [17635] = {
                icon = "Interface\\Icons\\inv_potion_62",
                source = "Drops from General Drakkisath in Upper Blackrock Spire",
            },
            [17636] = {
                icon = "Interface\\Icons\\inv_potion_97",
                source = "Drops from Balnazzar in Stratholme",
            },
            [17637] = {
                icon = "Interface\\Icons\\inv_potion_41",
                source = "Drops from Ras Frostwhisper in Scholomance",
            },
            [17638] = {
                icon = "Interface\\Icons\\inv_potion_48",
                source = "Drops from Gyth in Upper Blackrock Spire",
            },
            [25146] = {
                icon = "Interface\\Icons\\spell_fire_fire",
                source = "Sold by Lokhtos Darkbargainer, Blackrock Depths (requires Thorium Brotherhood - Friendly)",
            },
            [17634] = {
                icon = "Interface\\Icons\\inv_potion_26",
                source = "Sold by Pix Xizzix (Stranglethorn Vale); also world drop",
            },
            [22732] = {
                icon = "Interface\\Icons\\inv_potion_47",
                source = "Dropped by Molten Core bosses",
            },
        },
    }
)
