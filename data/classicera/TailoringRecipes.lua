TrainerSpellsBuiltin_ProfessionRecipe = TrainerSpellsBuiltin_ProfessionRecipe or {}
TrainerSpellsBuiltin_ProfessionRecipe.Tailoring = TrainerSpellsBuiltin_ProfessionRecipe.Tailoring or {
    [40] = {
        [2389] = {
            icon = "Interface\\Icons\\inv_chest_cloth_18",
            source = "World drop",
        },
    },
    [55] = {
        [7629] = {
            icon = "Interface\\Icons\\inv_chest_cloth_40",
            source = "World drop",
        },
        [7630] = {
            icon = "Interface\\Icons\\inv_chest_cloth_37",
            source = "Sold by Tharynn Bouden (Elwynn Forest, Goldshire), Valdaron (Darkshore, Auberdine), Borya (Orgrimmar), Constance Brisboise (Tirisfal Glades), Wrahk (Barrens, Crossroads)",
        },
    },
    [70] = {
        [6686] = {
            icon = "Interface\\Icons\\inv_misc_bag_02",
            source = "Sold by Valdaron (Darkshore, Auberdine), Gina MacGregor (Westfall), Mahu (Thunder Bluff), Andrew Hilbert (Silverpine Forest)",
        },
        [7633] = {
            icon = "Interface\\Icons\\inv_chest_cloth_23",
            source = "Sold by Elynna (Darnassus), Drake Lindgren (Elwynn Forest), Andrew Hilbert (Silverpine Forest)",
        },
    },
    [95] = {
        [3758] = {
            icon = "Interface\\Icons\\inv_misc_bag_10_green",
            source = "World drop",
        },
        [3847] = {
            icon = "Interface\\Icons\\inv_boots_09",
            source = "World drop",
        },
    },
    [100] = {
        [3844] = {
            icon = "Interface\\Icons\\inv_misc_cape_14",
            source = "World drop",
        },
        [7639] = {
            icon = "Interface\\Icons\\inv_shirt_13",
            source = "Sold by Alexandra Bolero (Stormwind), Gina MacGregor (Westfall), Borya (Orgrimmar), Yonada (Barrens, Camp Taurajo), Mallen Swain (Hillsbrad Foothills, Tarren Mill)",
        },
    },
    [105] = {
        [2403] = {
            icon = "Interface\\Icons\\inv_chest_cloth_13",
            source = "World drop",
        },
    },
    [115] = {
        [6688] = {
            icon = "Interface\\Icons\\inv_misc_bag_10_red",
            source = "Sold by Valdaron (Darkshore), Rann Flamespinner (Loch Modan), Kiknikle (Barrens), Amy Davenport (Redridge Mountains), Jennabink Powerseam (Wetlands), Borya (Orgrimmar), Mahu (Thunder Bluff), Millie Gregorian (Undercity), Zixil (Hillsbrad Foothills), Wrahk (Barrens), Yonada (Barrens)",
        },
        [7643] = {
            icon = "Interface\\Icons\\inv_chest_cloth_24",
            source = "Sold by Elynna (Darnassus), Rann Flamespinner (Loch Modan), Ranik (Barrens, Ratchet), Sheri Zipstitch (Duskwood), Jennabink Powerseam (Wetlands)",
        },
    },
    [120] = {
        [3849] = {
            icon = "Interface\\Icons\\inv_shoulder_09",
            source = "World drop",
        },
        [7892] = {
            icon = "Interface\\Icons\\inv_shirt_blue_01",
            source = "World drop",
        },
        [7893] = {
            icon = "Interface\\Icons\\inv_shirt_green_01",
            source = "World drop",
        },
        [12047] = {
            icon = "Interface\\Icons\\inv_pants_02",
            source = "World drop",
        },
    },
    [125] = {
        [3851] = {
            icon = "Interface\\Icons\\inv_pants_06",
            source = "World drop",
        },
        [3868] = {
            icon = "Interface\\Icons\\inv_gauntlets_19",
            source = "World drop",
        },
    },
    [135] = {
        [3869] = {
            icon = "Interface\\Icons\\inv_shirt_yellow_01",
            source = "Sold by Danielle Zipstitch (Duskwood, Darkshire) (Alliance only; Horde must use the neutral auction house or Alliance help)",
        },
    },
    [140] = {
        [3856] = {
            icon = "Interface\\Icons\\inv_boots_03",
            source = "World drop",
        },
    },
    [145] = {
        [3854] = {
            icon = "Interface\\Icons\\inv_gauntlets_17",
            source = "Sold by Kireena (Desolace, Ghost Walker Post, Horde)",
        },
        [8780] = {
            icon = "Interface\\Icons\\inv_gauntlets_22",
            source = "World drop",
        },
    },
    [150] = {
        [6692] = {
            icon = "Interface\\Icons\\inv_chest_cloth_29",
            source = "Dropped by Defias Enchanter (Duskwood), Dark Strand Voidcaller (Darkshore)",
        },
        [8782] = {
            icon = "Interface\\Icons\\inv_gauntlets_16",
            source = "World drop",
        },
    },
    [155] = {
        [3870] = {
            icon = "Interface\\Icons\\inv_shirt_black_01",
            source = "Sold by Sheri Zipstitch (Duskwood), Xizk Goodstitch (Booty Bay, Stranglethorn Vale), Mallen Swain (Hillsbrad Foothills, Tarren Mill)",
        },
    },
    [165] = {
        [3857] = {
            icon = "Interface\\Icons\\inv_helmet_31",
            source = "Sold by Xizk Goodstitch (Booty Bay, Stranglethorn Vale)",
        },
        [8784] = {
            icon = "Interface\\Icons\\inv_chest_cloth_06",
            source = "World drop",
        },
    },
    [170] = {
        [3858] = {
            icon = "Interface\\Icons\\inv_helmet_27",
            source = "Dropped by mobs in Scarlet Monastery and Razorfen Kraul, also found in chests and lockboxes",
        },
    },
    [175] = {
        [3860] = {
            icon = "Interface\\Icons\\inv_boots_05",
            source = "World drop",
        },
        [6693] = {
            icon = "Interface\\Icons\\inv_misc_bag_07_green",
            source = "World drop",
        },
        [8786] = {
            icon = "Interface\\Icons\\inv_chest_cloth_14",
            source = "Sold by Brienna Starglow (Feralas), Jun'ha (Arathi Highlands, Hammerfall)",
        },
    },
    [180] = {
        [3863] = {
            icon = "Interface\\Icons\\inv_belt_25",
            source = "World drop",
        },
        [8789] = {
            icon = "Interface\\Icons\\inv_chest_cloth_16",
            source = "Sold by Xizk Goodstitch (Booty Bay, Stranglethorn Vale)",
        },
    },
    [185] = {
        [3872] = {
            icon = "Interface\\Icons\\inv_shirt_16",
            source = "Dropped by mobs in Scarlet Monastery, Rumbling Exile (Arathi Highlands), also found in chests and lockboxes",
        },
        [6695] = {
            icon = "Interface\\Icons\\inv_misc_bag_07_black",
            source = "Dropped by Syndicate Spy (Alterac Mountains), Shadowy Assassin (Hillsbrad Foothills), Theramore Infiltrator (Dustwallow Marsh)",
        },
    },
    [190] = {
        [8793] = {
            icon = "Interface\\Icons\\inv_shoulder_23",
            source = "World drop",
        },
        [8795] = {
            icon = "Interface\\Icons\\inv_shoulder_27",
            source = "World drop",
        },
        [21945] = {
            icon = "Interface\\Icons\\inv_shirt_green_01",
            source = "Found in Smokywood Pastures Special Gift and Ticking Present (Feast of Winter Veil)",
        },
    },
    [195] = {
        [8797] = {
            icon = "Interface\\Icons\\inv_belt_24",
            source = "World drop",
        },
    },
    [200] = {
        [3862] = {
            icon = "Interface\\Icons\\inv_misc_cape_04",
            source = "Sold by Micha Yance (Hillsbrad Foothills, Southshore), Ghok'kah (Dustwallow Marsh, Brackenwall Village)",
        },
        [3864] = {
            icon = "Interface\\Icons\\inv_belt_06",
            source = "Dropped by mobs in Scarlet Monastery and Zul'Farrak, Otto (Arathi Highlands), also found in chests",
        },
        [3873] = {
            icon = "Interface\\Icons\\inv_shirt_black_01",
            source = "Sold by Narkk (Stranglethorn Vale, Booty Bay)",
        },
    },
    [205] = {
        [8802] = {
            icon = "Interface\\Icons\\inv_chest_cloth_25",
            source = "Sold by Vizzklick (Tanaris, Gadgetzan)",
        },
    },
    [215] = {
        [12056] = {
            icon = "Interface\\Icons\\inv_shirt_09",
            source = "World drop",
        },
        [12059] = {
            icon = "Interface\\Icons\\inv_misc_cape_11",
            source = "World drop",
        },
        [12060] = {
            icon = "Interface\\Icons\\inv_pants_06",
            source = "World drop",
        },
    },
    [220] = {
        [12064] = {
            icon = "Interface\\Icons\\inv_shirt_orange_01",
            source = "Sold by Elynna (Darnassus), Mahu (Thunder Bluff)",
        },
    },
    [225] = {
        [12066] = {
            icon = "Interface\\Icons\\inv_gauntlets_19",
            source = "World drop",
        },
        [27658] = {
            icon = "Interface\\Icons\\inv_misc_bag_enchantedmageweave",
            source = "Sold by Tilli Thistlefuzz (Ironforge), Jessara Cordell (Stormwind), Kania (Silithus), Kithas (Orgrimmar), Nata Dawnstrider (Thunder Bluff), Thaddeus Webb (Undercity), Lilly (Silverpine Forest), Leo Sarn (Silverpine Forest)",
        },
    },
    [230] = {
        [12075] = {
            icon = "Interface\\Icons\\inv_shirt_purple_01",
            source = "Sold by Outfitter Eric (Ironforge), Borya (Orgrimmar)",
        },
    },
    [235] = {
        [12078] = {
            icon = "Interface\\Icons\\inv_shoulder_23",
            source = "World drop",
        },
        [12080] = {
            icon = "Interface\\Icons\\inv_shirt_red_01",
            source = "Sold by Outfitter Eric (Ironforge), Borya (Orgrimmar)",
        },
    },
    [240] = {
        [12081] = {
            icon = "Interface\\Icons\\inv_helmet_29",
            source = "Sold by Cowardly Crosby (Stranglethorn Vale, Booty Bay)",
        },
        [12084] = {
            icon = "Interface\\Icons\\inv_misc_bandana_03",
            source = "World drop",
        },
        [12085] = {
            icon = "Interface\\Icons\\inv_shirt_08",
            source = "Sold by Outfitter Eric (Ironforge), Millie Gregorian (Undercity)",
        },
    },
    [245] = {
        [12086] = {
            icon = "Interface\\Icons\\inv_helmet_27",
            source = "Quest reward: \"The Undermarket\"",
        },
        [12089] = {
            icon = "Interface\\Icons\\inv_pants_01",
            source = "Sold by Outfitter Eric (Ironforge), Millie Gregorian (Undercity)",
        },
    },
    [250] = {
        [12091] = {
            icon = "Interface\\Icons\\inv_chest_cloth_04",
            source = "Sold by Alexandra Bolero (Stormwind), Mahu (Thunder Bluff)",
        },
        [12093] = {
            icon = "Interface\\Icons\\inv_shirt_black_01",
            source = "Sold by Outfitter Eric (Ironforge), Millie Gregorian (Undercity)",
        },
        [18560] = {
            icon = "Interface\\Icons\\inv_fabric_moonrag_01",
            source = "Sold by Qia (Winterspring, Everlook)",
        },
        [26403] = {
            icon = "Interface\\Icons\\inv_chest_cloth_56",
            source = "Quest reward: \"Festive Recipes\" (Lunar Festival)",
        },
        [26407] = {
            icon = "Interface\\Icons\\inv_chest_cloth_55",
            source = "Quest reward: \"Festive Recipes\" (Lunar Festival)",
        },
    },
    [255] = {
        [18403] = {
            icon = "Interface\\Icons\\inv_chest_cloth_08",
            source = "World drop (level 50+ mobs), also found in chests and lockboxes",
        },
        [18404] = {
            icon = "Interface\\Icons\\inv_chest_cloth_03",
            source = "World drop",
        },
    },
    [260] = {
        [18405] = {
            icon = "Interface\\Icons\\inv_misc_bag_19",
            source = "Sold by Qia (Winterspring, Everlook)",
        },
        [18406] = {
            icon = "Interface\\Icons\\inv_chest_cloth_04",
            source = "Sold by Darnall (Moonglade, Nighthaven)",
        },
        [18407] = {
            icon = "Interface\\Icons\\inv_chest_cloth_04",
            source = "World drop",
        },
        [18408] = {
            icon = "Interface\\Icons\\inv_chest_cloth_07",
            source = "Dropped by Twilight Fire Guard (Searing Gorge)",
        },
        [26085] = {
            icon = "Interface\\Icons\\inv_misc_bag_soulbag",
            source = "Sold by Vizzklick (Tanaris, Gadgetzan)",
        },
    },
    [265] = {
        [18409] = {
            icon = "Interface\\Icons\\inv_misc_cape_10",
            source = "Sold by Darnall (Moonglade, Nighthaven)",
        },
        [18410] = {
            icon = "Interface\\Icons\\inv_belt_16",
            source = "Dropped by Lingering Highborne (Azshara)",
        },
        [18411] = {
            icon = "Interface\\Icons\\inv_gauntlets_05",
            source = "World drop",
        },
    },
    [270] = {
        [18412] = {
            icon = "Interface\\Icons\\inv_gauntlets_05",
            source = "Dropped by Twilight Fire Guard (Searing Gorge)",
        },
        [18413] = {
            icon = "Interface\\Icons\\inv_gauntlets_05",
            source = "Dropped by Lingering Highborne (Azshara), Unseen Servant (Eastern Plaguelands)",
        },
        [18414] = {
            icon = "Interface\\Icons\\inv_chest_cloth_26",
            source = "World drop",
        },
        [18415] = {
            icon = "Interface\\Icons\\inv_gauntlets_21",
            source = "World drop",
        },
    },
    [275] = {
        [18416] = {
            icon = "Interface\\Icons\\inv_shirt_08",
            source = "Dropped by Unseen Servant (Eastern Plaguelands)",
        },
        [18417] = {
            icon = "Interface\\Icons\\inv_gauntlets_25",
            source = "Sold by Qia (Winterspring, Everlook)",
        },
        [18418] = {
            icon = "Interface\\Icons\\inv_misc_cape_18",
            source = "Dropped by Thaurissan Firewalker (Burning Steppes)",
        },
        [18419] = {
            icon = "Interface\\Icons\\inv_pants_06",
            source = "Sold by Lorelae Wintersong (Moonglade)",
        },
        [18420] = {
            icon = "Interface\\Icons\\inv_misc_cape_11",
            source = "World drop",
        },
        [18421] = {
            icon = "Interface\\Icons\\inv_pants_09",
            source = "Dropped by Dark Summoner (Eastern Plaguelands)",
        },
        [18422] = {
            icon = "Interface\\Icons\\inv_misc_cape_18",
            source = "Dropped by Overmaster Pyron (rare mob)",
        },
        [27659] = {
            icon = "Interface\\Icons\\inv_misc_bag_enchantedrunecloth",
            source = "Sold by Kania (Silithus)",
        },
        [27724] = {
            icon = "Interface\\Icons\\inv_misc_bag_cenarionherbbag",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Friendly)",
        },
    },
    [280] = {
        [18423] = {
            icon = "Interface\\Icons\\inv_boots_05",
            source = "Sold by Darnall (Moonglade, Nighthaven)",
        },
        [18424] = {
            icon = "Interface\\Icons\\inv_pants_08",
            source = "World drop",
        },
        [18434] = {
            icon = "Interface\\Icons\\inv_pants_06",
            source = "Dropped by Thaurissan Firewalker (Burning Steppes)",
        },
    },
    [285] = {
        [18436] = {
            icon = "Interface\\Icons\\inv_chest_cloth_49",
            source = "Dropped by Cobalt Mageweaver (Winterspring)",
        },
        [18437] = {
            icon = "Interface\\Icons\\inv_boots_05",
            source = "World drop",
        },
        [18438] = {
            icon = "Interface\\Icons\\inv_pants_09",
            source = "World drop",
        },
        [22813] = {
            icon = "Interface\\Icons\\inv_chest_chain_14",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
        [26086] = {
            icon = "Interface\\Icons\\inv_misc_bag_felclothbag",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
    },
    [290] = {
        [18439] = {
            icon = "Interface\\Icons\\inv_pants_09",
            source = "World drop",
        },
        [18440] = {
            icon = "Interface\\Icons\\inv_pants_13",
            source = "World drop (level 55+ mobs), also found in chests and lockboxes",
        },
        [18441] = {
            icon = "Interface\\Icons\\inv_pants_01",
            source = "Dropped by Spectral Citizen (Stratholme)",
        },
        [18442] = {
            icon = "Interface\\Icons\\inv_helmet_34",
            source = "World drop",
        },
        [19435] = {
            icon = "Interface\\Icons\\inv_boots_05",
            source = "Quest reward: \"Sacred Cloth\"",
        },
        [23662] = {
            icon = "Interface\\Icons\\inv_belt_09",
            source = "Sold by Meilosh, Felwood (requires Timbermaw Hold - Honored)",
        },
        [23664] = {
            icon = "Interface\\Icons\\inv_boots_cloth_03",
            source = "Sold by Argent Quartermaster Hasana (Tirisfal Glades), Lightspark (Western Plaguelands) & Miranda Breechlock (Eastern Plaguelands) (requires Argent Dawn - Honored)",
        },
    },
    [295] = {
        [18444] = {
            icon = "Interface\\Icons\\inv_misc_bandana_03",
            source = "World drop",
        },
    },
    [300] = {
        [18445] = {
            icon = "Interface\\Icons\\inv_misc_bag_14",
            source = "World drop",
        },
        [18446] = {
            icon = "Interface\\Icons\\inv_chest_cloth_46",
            source = "Dropped by Dark Caster (Eastern Plaguelands)",
        },
        [18447] = {
            icon = "Interface\\Icons\\inv_chest_cloth_08",
            source = "World drop (level 60 mobs: Blackrock Spire, Azuregos, Onyxia)",
        },
        [18448] = {
            icon = "Interface\\Icons\\inv_shoulder_02",
            source = "World drop (level 55+ mobs), also found in chests and lockboxes",
        },
        [18449] = {
            icon = "Interface\\Icons\\inv_shoulder_21",
            source = "World drop",
        },
        [18450] = {
            icon = "Interface\\Icons\\inv_helmet_63",
            source = "Dropped by Dark Caster (Eastern Plaguelands)",
        },
        [18451] = {
            icon = "Interface\\Icons\\inv_chest_cloth_09",
            source = "World drop",
        },
        [18452] = {
            icon = "Interface\\Icons\\inv_misc_bandana_01",
            source = "World drop (raid bosses and level 60 mobs: Azuregos, Onyxia, Nefarian, Molten Core, Zul'Gurub)",
        },
        [18453] = {
            icon = "Interface\\Icons\\inv_shoulder_23",
            source = "World drop",
        },
        [18454] = {
            icon = "Interface\\Icons\\inv_gauntlets_06",
            source = "World drop (raid bosses and level 60 mobs: Azuregos, Onyxia, Stratholme, Blackrock Spire)",
        },
        [18455] = {
            icon = "Interface\\Icons\\inv_misc_bag_13",
            source = "World drop (raid bosses and level 60 mobs: Azuregos, Onyxia, Nefarian, Stratholme, Zul'Gurub)",
        },
        [18456] = {
            icon = "Interface\\Icons\\inv_chest_cloth_31",
            source = "Dropped by Grand Crusader Dathrohan / Balnazzar (Stratholme)",
        },
        [18457] = {
            icon = "Interface\\Icons\\inv_chest_cloth_38",
            source = "Dropped by Firebrand Pyromancer (Upper Blackrock Spire)",
        },
        [18458] = {
            icon = "Interface\\Icons\\inv_chest_cloth_51",
            source = "Dropped by Darkmaster Gandling (Scholomance)",
        },
        [20848] = {
            icon = "Interface\\Icons\\inv_shoulder_23",
            source = "Sold by Lokhtos Darkbargainer (Blackrock Depths, Thorium Brotherhood rep, requires Honored)",
        },
        [20849] = {
            icon = "Interface\\Icons\\inv_gauntlets_26",
            source = "Sold by Lokhtos Darkbargainer, Blackrock Depths (requires Thorium Brotherhood - Friendly)",
        },
        [22759] = {
            icon = "Interface\\Icons\\inv_bracer_09",
            source = "Dropped by Molten Core bosses",
        },
        [22866] = {
            icon = "Interface\\Icons\\inv_belt_31",
            source = "Found in Knot Thimblejack's Cache (Dire Maul chest)",
        },
        [22867] = {
            icon = "Interface\\Icons\\inv_gauntlets_19",
            source = "Found in Knot Thimblejack's Cache (Dire Maul chest)",
        },
        [22868] = {
            icon = "Interface\\Icons\\inv_gauntlets_19",
            source = "Found in Knot Thimblejack's Cache (Dire Maul chest)",
        },
        [22869] = {
            icon = "Interface\\Icons\\inv_gauntlets_17",
            source = "Found in Knot Thimblejack's Cache (Dire Maul chest)",
        },
        [22870] = {
            icon = "Interface\\Icons\\inv_misc_cape_06",
            source = "Found in Knot Thimblejack's Cache (Dire Maul chest)",
        },
        [22902] = {
            icon = "Interface\\Icons\\inv_chest_cloth_04",
            source = "Sold by Shen'dralar Provisioner (Dire Maul, Library)",
        },
        [23663] = {
            icon = "Interface\\Icons\\inv_shoulder_19",
            source = "Sold by Meilosh, Felwood (requires Timbermaw Hold - Revered)",
        },
        [23665] = {
            icon = "Interface\\Icons\\inv_shoulder_13",
            source = "Sold by Argent Quartermaster Hasana (Tirisfal Glades), Lightspark (Western Plaguelands) & Miranda Breechlock (Eastern Plaguelands) (requires Argent Dawn - Revered)",
        },
        [23666] = {
            icon = "Interface\\Icons\\inv_chest_cloth_18",
            source = "Sold by Lokhtos Darkbargainer (Blackrock Depths, Thorium Brotherhood rep, requires Honored)",
        },
        [23667] = {
            icon = "Interface\\Icons\\inv_pants_06",
            source = "Sold by Lokhtos Darkbargainer (Blackrock Depths, Thorium Brotherhood rep, requires Revered)",
        },
        [24091] = {
            icon = "Interface\\Icons\\inv_chest_cloth_07",
            source = "Sold by Rin'wosho the Trader (Stranglethorn Vale, Zandalar Tribe rep, requires Revered)",
        },
        [24092] = {
            icon = "Interface\\Icons\\inv_pants_cloth_14",
            source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Honored)",
        },
        [24093] = {
            icon = "Interface\\Icons\\inv_boots_cloth_02",
            source = "Sold by Rin'wosho the Trader, Stranglethorn Vale (requires Zandalar Tribe - Friendly)",
        },
        [24901] = {
            icon = "Interface\\Icons\\inv_pants_cloth_05",
            source = "Found in Decoded True Believer Clippings (Scholomance)",
        },
        [24902] = {
            icon = "Interface\\Icons\\inv_belt_14",
            source = "Found in Decoded True Believer Clippings (Scholomance)",
        },
        [24903] = {
            icon = "Interface\\Icons\\inv_boots_05",
            source = "Found in Decoded True Believer Clippings (Scholomance)",
        },
        [26087] = {
            icon = "Interface\\Icons\\inv_misc_bag_corefelclothbag",
            source = "Dropped by Molten Core bosses",
        },
        [27660] = {
            icon = "Interface\\Icons\\inv_misc_bag_bigbagofenchantments",
            source = "Dropped by Magister Kalendris (Dire Maul)",
        },
        [27725] = {
            icon = "Interface\\Icons\\inv_misc_bag_satchelofcenarius",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Revered)",
        },
        [28205] = {
            icon = "Interface\\Icons\\inv_gauntlets_06",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
        [28207] = {
            icon = "Interface\\Icons\\inv_chest_cloth_08",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
        [28208] = {
            icon = "Interface\\Icons\\inv_misc_cape_16",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
        [28209] = {
            icon = "Interface\\Icons\\inv_bracer_07",
            source = "Unknown source (no source listed on Wowhead Classic)",
        },
        [28210] = {
            icon = "Interface\\Icons\\inv_misc_cape_17",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Revered)",
        },
        [28480] = {
            icon = "Interface\\Icons\\inv_chest_plate07",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Revered)",
        },
        [28481] = {
            icon = "Interface\\Icons\\inv_crown_01",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Honored)",
        },
        [28482] = {
            icon = "Interface\\Icons\\inv_shoulder_18",
            source = "Sold by Mishta (Silithus, Cenarion Circle rep, requires Friendly)",
        },
    },
}
